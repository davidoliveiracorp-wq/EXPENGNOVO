    ...
    <div className={`h-full flex flex-col ${bg}`}>
      ...
      <div className={`flex items-center justify-between px-6 py-4 border-b flex-shrink-0 ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
        ...
        <div className="flex items-center gap-2">
          <button onClick={openShareModal} className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-xl text-sm font-medium transition-colors">
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-start justify-center p-4 pt-12 overflow-y-auto">
          <div className={`rounded-2xl w-full max-w-lg shadow-2xl mb-8 ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
            ...
            <div className="p-6 space-y-5">
              ...
              {/* What to include — always visible */}
              <div>
                ...
                <div className={`rounded-xl border overflow-hidden divide-y ${isDark ? 'border-gray-700 divide-gray-700' : 'border-gray-200 divide-gray-100'}`}>
                  <label className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors ${isDark ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}>
                    <input type="checkbox" checked={waIncludeYT} onChange={(e) => setWaIncludeYT(e.target.checked)} className="accent-red-500 w-4 h-4 flex-shrink-0" />
                  ...
                  <label className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors ${isDark ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}>
                    <input type="checkbox" checked={waIncludeLetra} onChange={(e) => setWaIncludeLetra(e.target.checked)} className="accent-orange-500 w-4 h-4 flex-shrink-0" />
                    <span className="text-base flex-shrink-0">📝</span>
                    <span className={`text-sm ${heading}`}>Letra completa</span>
                  </label>
                  <label className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors ${isDark ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}>
                    <input type="checkbox" checked={waIncludeCifra} onChange={(e) => setWaIncludeCifra(e.target.checked)} className="accent-orange-500 w-4 h-4 flex-shrink-0" />
                    <span className="text-base flex-shrink-0">🎸</span>
